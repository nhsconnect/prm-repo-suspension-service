locals {
  ...
  environment_variables = [
    ...
    { name = "INCOMING_QUEUE_NAME", value = var.is_end_of_transfer_service ? aws_sqs_queue.transfer_complete[0].name : aws_sqs_queue.suspensions.name },
    ...
    { name = "REPO_INCOMING_SNS_TOPIC_ARN", value = var.is_end_of_transfer_service ? "" : aws_sns_topic.repo_incoming[0].arn },
